	...
	const mockDeals = [
		{
			...
			DATE_CREATE: "2024-02-15T10:00:00Z",
			ASSIGNED_BY_ID: "1",
		},
		{
			ID: "102",
			...
			CLOSED: "N",
			CLOSEDATE: null,
			DATE_CREATE: "2024-03-01T14:30:00Z",
			ASSIGNED_BY_ID: "2",
		},
		{
			ID: "103",
			...
			DATE_CREATE: "2024-02-20T09:15:00Z",
			ASSIGNED_BY_ID: "3",
		},
		{
			ID: "104",
			...
			CLOSED: "N",
			CLOSEDATE: null,
			DATE_CREATE: "2024-03-10T11:45:00Z",
			ASSIGNED_BY_ID: "1",
		},
		{
			ID: "105",
			...
			DATE_CREATE: "2024-04-01T13:20:00Z",
			ASSIGNED_BY_ID: "2",
		},
		{
			ID: "106",
			...
			DATE_CREATE: "2024-03-15T15:00:00Z",
			ASSIGNED_BY_ID: "3",
		},
		{
			ID: "107",
			...
			CLOSED: "N",
			CLOSEDATE: null,
			DATE_CREATE: "2024-04-05T10:30:00Z",
			ASSIGNED_BY_ID: "1",
		},
		{
			ID: "108",
			...
			DATE_CREATE: "2024-02-28T09:45:00Z",
			ASSIGNED_BY_ID: "2",
		},
		{
			ID: "109",
			...
			CLOSED: "N",
			CLOSEDATE: null,
			DATE_CREATE: "2024-03-20T14:15:00Z",
			ASSIGNED_BY_ID: "3",
		},
	]
		...
		callMethod: (method, params, callback) => {
			...
			if (method === "user.get") {
				...
				if (params.filter) {
					...
					if (params.filter[">=DATE_CREATE"]) {
						const minDate = new Date(params.filter[">=DATE_CREATE"])
						filteredDeals = filteredDeals.filter(
							deal => deal.DATE_CREATE && new Date(deal.DATE_CREATE) >= minDate
						)
					}

					if (params.filter["<=DATE_CREATE"]) {
						const maxDate = new Date(params.filter["<=DATE_CREATE"])
						filteredDeals = filteredDeals.filter(
							deal => deal.DATE_CREATE && new Date(deal.DATE_CREATE) <= maxDate
						)
					}

					if (params.filter.STATUS_ID) {
						filteredDeals = filteredDeals.filter(
							deal => deal.STAGE_ID === params.filter.STATUS_ID
						)
					}
				}
				...
				if (method === "user.get") {
					...
					if (params.filter) {
						...
						if (params.filter[">=DATE_CREATE"]) {
							...
							filteredDeals = filteredDeals.filter(
								deal =>
									deal.DATE_CREATE && new Date(deal.DATE_CREATE) >= minDate
							)
						}
						...
						if (params.filter["<=DATE_CREATE"]) {
							...
							filteredDeals = filteredDeals.filter(
								deal =>
									deal.DATE_CREATE && new Date(deal.DATE_CREATE) <= maxDate
							)
						}
